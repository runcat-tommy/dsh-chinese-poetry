...
- **视图体验**：搜索与筛选分区卡片化（搜索框与按钮合为一体、快捷操作独立胶囊行、筛选控件收进工具条卡片）；首次进入显示空态引导卡片，示例词一点即搜；AI 解读后自动切到「对话」视图；诗词视图内容在对话 / 轨迹 / 诗词间切换时保留；最近搜索区块恒定显示。
...
- [x] M6：UI 改版（v1.3.0）—— 搜索框与按钮合为一体、快捷操作独立胶囊行、筛选控件收进工具条卡片、首屏空态引导卡片、统一设计 token 与主次按钮分级
