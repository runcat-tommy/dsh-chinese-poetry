...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PLUGIN_VERSION = "1.4.0"; // keep in sync with package.json
    ...
    var zh = {
      ...
      toolFilter: "筛一首",
      toolFeihua: "飞花令",
      toolHintSearch: "输入关键词搜索 37 万首全库；2 字以内仅搜本地精选。",
      toolHintFilter: "按朝代 / 体裁 / 作者随机取一首。",
      toolHintFeihua: "输入 1 个字，随机给出含该字的诗，可连点换诗。",
      reset: "重置",
      all: "全部",
      toHant: "繁體",
      toHans: "简体",
    ...
    var en = {
      ...
      emptyTitle: "Search 370,000+ classical Chinese poems",
      ...
      filterType: "Genre",
      filterAuthor: "Author",
      filterGo: "Random pick",
      toolSearch: "Search poems",
      toolFilter: "Filter & random",
      toolFeihua: "Feihua",
      toolHintSearch: "Search the 370k-poem corpus by keyword; 2-character queries use local picks only.",
      toolHintFilter: "Random pick by dynasty / genre / author.",
      toolHintFeihua: "Type one character for random poems containing it; click again for another.",
      reset: "Reset",
      all: "All",
      toHant: "繁體",
      toHans: "简体",
...
.cp-root {
  ...
  --cp-radius: 10px;
  --cp-radius-sm: 8px;
  --cp-line: var(--dsw-alias-border-l1);
  --cp-surface: var(--dsw-alias-bg-layer-1);
  --cp-surface-2: var(--dsw-alias-bg-layer-2);
  --cp-hover: var(--dsw-alias-interactive-bg-hover, var(--dsw-alias-bg-layer-2));
  --cp-muted: var(--dsw-alias-label-tertiary);
  --cp-faint: var(--dsw-alias-label-secondary);
}
.cp-hero {
  display: flex; flex-direction: column; gap: 12px;
  padding: 14px; border: 1px solid var(--cp-line); border-radius: var(--cp-radius);
  background: var(--cp-surface);
}
.cp-searchrow { display: flex; align-items: stretch; max-width: 720px; }
.cp-search {
  flex: 1; min-width: 0;
  background: var(--cp-surface-2);
  border: 1px solid var(--cp-line); border-right: 0;
  border-radius: var(--cp-radius) 0 0 var(--cp-radius);
  padding: 10px 14px; font-size: 14px; color: inherit;
  transition: border-color .15s;
}
.cp-search::placeholder { color: var(--cp-muted); }
.cp-search:focus { border-color: var(--dsw-alias-brand-primary); outline: none; }
.cp-btn.cp-search-btn { border-radius: 0 var(--cp-radius) var(--cp-radius) 0; padding: 10px 22px; font-size: 14px; flex: none; }
.cp-quick { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin-top: 12px; }
.cp-quick .cp-btn { border-radius: 999px; padding: 5px 14px; font-size: 12.5px; color: var(--cp-faint); }
.cp-quick .cp-btn:hover:not(:disabled) { color: var(--dsw-alias-label-primary); }
.cp-btn {
  background: transparent; border: 1px solid var(--cp-line); color: var(--dsw-alias-label-primary);
  border-radius: var(--cp-radius-sm); padding: 6px 13px; font-size: 13px; cursor: pointer;
...
.cp-btn.toggle-on {
  background: var(--cp-hover); border-color: var(--dsw-alias-brand-primary);
  color: var(--dsw-alias-brand-primary); font-weight: 500;
}
.cp-lang-btn { flex: none; }
.cp-modes {
  display: inline-flex; gap: 4px; padding: 3px; align-self: flex-start;
  background: var(--cp-surface-2); border: 1px solid var(--cp-line); border-radius: 999px;
}
.cp-mode {
  border: 0; background: transparent; color: var(--cp-faint); border-radius: 999px;
  padding: 6px 16px; font-size: 13px; cursor: pointer; white-space: nowrap;
  transition: background .15s, color .15s;
}
.cp-mode:hover { color: var(--dsw-alias-label-primary); }
.cp-mode.is-active {
  background: var(--dsw-alias-button-primary-fill, var(--dsw-alias-label-primary));
  color: var(--dsw-alias-label-primary-foreground, var(--dsw-alias-bg-layer-3));
  font-weight: 500;
}
.cp-panel { min-height: 84px; }
.cp-panelbody { display: flex; flex-direction: column; gap: 10px; }
.cp-panelhint { font-size: 12px; color: var(--cp-muted); }
.cp-filterrow { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }
.cp-feihuarow { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; }
.cp-select, .cp-author-input, .cp-feihua {
  background: var(--cp-surface-2); border: 1px solid var(--cp-line); color: inherit;
  border-radius: var(--cp-radius-sm); padding: 6px 9px; font-size: 12.5px;
}
.cp-select { max-width: 150px; }
.cp-author-input { width: 120px; }
.cp-feihua { width: 64px; font-size: 16px; text-align: center; padding: 7px 0; }
.cp-select:focus, .cp-author-input:focus, .cp-feihua:focus { border-color: var(--dsw-alias-brand-primary); outline: none; }
.cp-chips { display: flex; gap: 6px; align-items: center; flex-wrap: wrap; font-size: 12px; color: var(--cp-muted); }
.cp-chip {
  background: transparent; border: 1px solid var(--cp-line); color: var(--cp-faint);
  border-radius: 999px; padding: 3px 11px; font-size: 12px; cursor: pointer;
  transition: background .15s, color .15s, border-color .15s;
}
.cp-chip:hover { background: var(--cp-hover); color: var(--dsw-alias-label-primary); border-color: var(--dsw-alias-brand-primary); }
.cp-history {
  display: flex; gap: 6px; align-items: center; flex-wrap: wrap;
  margin-top: 12px; font-size: 12px; color: var(--cp-muted);
}
.cp-history-empty { opacity: .8; }
.cp-status {
  margin-top: 14px; font-size: 12px; color: var(--cp-muted);
  display: flex; gap: 14px; align-items: center; flex-wrap: wrap;
}
.cp-status.warn { color: var(--dsw-alias-state-warn-primary); }
.cp-note {
  margin-top: 10px; padding: 9px 13px; border-radius: var(--cp-radius-sm);
  font-size: 12.5px; line-height: 1.65;
  background: var(--cp-surface); border: 1px solid var(--cp-line);
  color: var(--dsw-alias-state-warn-primary);
}
.cp-msg {
  margin-top: 12px; padding: 10px 14px; border-radius: var(--cp-radius-sm);
  font-size: 13px; background: var(--cp-surface); border: 1px solid var(--cp-line);
  color: var(--cp-faint);
}
.cp-msg.error { color: var(--dsw-alias-state-error-primary); border-color: var(--dsw-alias-state-error-secondary); }
.cp-ai-note {
  margin-top: 10px; padding: 9px 13px; border-radius: var(--cp-radius-sm); font-size: 12.5px;
  background: var(--dsw-alias-state-success-tertiary, var(--cp-surface));
  border: 1px solid var(--dsw-alias-state-success-secondary, var(--cp-line));
  color: var(--dsw-alias-state-success-primary, var(--cp-faint));
}
.cp-empty {
  margin-top: 20px; padding: 34px 20px; text-align: center;
  border: 1px dashed var(--cp-line); border-radius: var(--cp-radius);
  background: var(--cp-surface);
}
.cp-empty-title { font-size: 14px; color: var(--cp-faint); margin-bottom: 14px; }
.cp-empty-chips { display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; }
...
.cp-row {
  ...
  background: var(--cp-surface); transition: border-color .15s;
}
.cp-row:hover { border-color: var(--dsw-alias-brand-primary); }
.cp-row-head { display: flex; align-items: center; gap: 10px; padding: 10px 14px; cursor: pointer; }
.cp-row-head:hover { background: var(--cp-hover); }
.cp-row-title { font-size: 14.5px; font-weight: 600; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.cp-row-meta { font-size: 12px; color: var(--cp-faint); display: flex; gap: 8px; flex: none; }
.cp-row-preview { font-size: 12.5px; color: var(--cp-muted); flex: 1; min-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.cp-row-body { padding: 6px 14px 12px; border-top: 1px solid var(--cp-line); }
.cp-lines { font-size: 14.5px; line-height: 2; margin: 10px 0; }
.cp-actions { display: flex; gap: 8px; flex-wrap: wrap; }
.cp-corrected {
  font-size: 11px; color: var(--dsw-alias-state-warn-primary);
  border: 1px solid var(--dsw-alias-state-warn-secondary);
  border-radius: 4px; padding: 0 5px; line-height: 1.6;
}
.cp-favbar { display: flex; align-items: center; gap: 8px; margin-top: 12px; }
.cp-festival-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(128px, 1fr)); gap: 10px; margin-top: 14px; }
.cp-festival-card {
  border: 1px solid var(--cp-line); border-radius: var(--cp-radius);
  background: var(--cp-surface); padding: 16px 10px; text-align: center; cursor: pointer;
  transition: background .15s, border-color .15s;
}
.cp-festival-card:hover { background: var(--cp-hover); border-color: var(--dsw-alias-brand-primary); }
.cp-festival-card.active { border-color: var(--dsw-alias-brand-primary); background: var(--cp-hover); }
.cp-festival-name { display: block; font-size: 15px; font-weight: 600; color: var(--dsw-alias-label-primary); }
.cp-festival-char { display: block; font-size: 12px; color: var(--cp-muted); margin-top: 5px; }
.cp-festival-actions { display: flex; gap: 8px; margin-top: 14px; align-items: center; flex-wrap: wrap; }
.cp-footer {
  margin-top: 26px; padding-top: 12px; border-top: 1px solid var(--cp-line);
  display: flex; flex-direction: column; gap: 4px;
}
.cp-credit { font-size: 11px; color: var(--cp-muted); line-height: 1.65; margin: 0; }
.cp-credit-link { color: var(--dsw-alias-state-info-primary); text-decoration: none; }
.cp-credit-link:hover { text-decoration: underline; }
.cp-version { font-size: 11px; color: var(--cp-muted); font-family: var(--dsw-font-mono, monospace); margin: 0; }
...
`;
    ...
    /** Sample keywords shown in the empty state so the view is never a blank canvas. */
    var SAMPLE_QUERIES = ["明月", "春风", "江南", "相思", "登高", "黄河"];
    ...
    function PoetryView(props) {
      ...
      var [tool, setTool] = useViewMemory(sessionKey, "tool", "search"); // 'search' | 'filter' | 'feihua' — one tool panel at a time
      ...
      var TOOL_TABS = [
        { id: "search", label: t("toolSearch") },
        { id: "filter", label: t("toolFilter") },
        { id: "feihua", label: t("toolFeihua") },
      ];

      return el("div", { className: "cp-root" },
        el("div", { className: "cp-hero" },
          el("div", { className: "cp-modes", role: "tablist" },
            TOOL_TABS.map(function (tb) {
              return el("button", {
                key: tb.id,
                type: "button",
                role: "tab",
                "aria-selected": tool === tb.id ? "true" : "false",
                className: "cp-mode" + (tool === tb.id ? " is-active" : ""),
                onClick: function () { setTool(tb.id); },
              }, tb.label);
            }),
          ),
          el("div", { className: "cp-panel" },
            tool === "search"
              ? el("div", { className: "cp-panelbody" },
                  el("div", { className: "cp-searchrow" },
                    el("input", {
                      className: "cp-search",
                      type: "text",
                    ...
                    el("button", { className: "cp-btn primary cp-search-btn", onClick: function () { runSearch(query); } }, t("search")),
                  ),
                  el("div", { className: "cp-panelhint" }, t("toolHintSearch")),
                  viewMode === "search"
            ...
            tool === "filter"
              ? el("div", { className: "cp-panelbody" },
                  ...
                  el("div", { className: "cp-chips" },
                    el("span", null, t("commonAuthors")),
                  ...
                  el("div", { className: "cp-panelhint" }, t("toolHintFilter")),
                )
              : null,
            tool === "feihua"
              ? el("div", { className: "cp-panelbody" },
                  el("div", { className: "cp-feihuarow" },
                    el("input", {
                      className: "cp-feihua",
                      type: "text",
                      maxLength: "1",
                    ...
                    el("button", { className: "cp-btn primary", onClick: function () { runFeihua(feihuaChar); } }, t("feihuaGo")),
                    el("span", { className: "cp-panelhint" }, t("toolHintFeihua")),
                  ),
                )
              : null,
          ),
        ),
        el("div", { className: "cp-quick" },
          ...
          el("button", {
            className: "cp-btn" + (viewMode === "festival" ? " toggle-on" : ""),
            onClick: function () { setViewMode(viewMode === "festival" ? "search" : "festival"); },
          }, t("festival")),
          el("button", {
            className: "cp-btn" + (viewMode === "favs" ? " toggle-on" : ""),
        ...
        (source || loading || copied || hasFilters || st.cacheOnly || message || note)
          ...
          : (viewMode === "favs"
              ...
              : (items && items.length > 0
                  ...
                  : (source || note
                      ...
                      : el("div", { className: "cp-empty" },
                          el("div", { className: "cp-empty-title" }, t("emptyTitle")),
                          el("div", { className: "cp-empty-chips" },
                            el("span", null, t("emptyTry")),
                            SAMPLE_QUERIES.map(function (w) {
                              return el("button", {
                                key: w,
                                className: "cp-chip",
                                onClick: function () { setQuery(w); runSearch(w); },
                              }, w);
                            }),
                          ),
                        )))),
