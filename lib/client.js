...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PLUGIN_VERSION = "1.2.1"; // keep in sync with package.json
...
.cp-credit { font-size: 11px; color: var(--dsw-alias-label-tertiary); line-height: 1.6; margin: 0 0 8px; }
...
`;
    ...
    function shareCardDataUrl(poem) {
      ...
      cv.width = 660; cv.height = 860;
      var g = cv.getContext("2d");
      if (!g) return null;
      try {
        // parchment background + double border
        g.fillStyle = "#f7f2e6"; g.fillRect(0, 0, 660, 860);
        g.strokeStyle = "#c9b28a"; g.lineWidth = 3; g.strokeRect(22, 22, 616, 816);
        g.strokeStyle = "#e0d3ba"; g.lineWidth = 1; g.strokeRect(32, 32, 596, 796);
        g.textAlign = "center";
        // title
        g.fillStyle = "#2b2620"; g.font = "bold 46px 'Kaiti SC','KaiTi','STKaiti',serif";
        wrapCanvasLines(g, poem.title || "无题", 330, 150, 540, 62, 3);
        // author · dynasty
        ...
        g.fillText((author || "") + (dynasty ? " · " + dynasty : ""), 330, 315);
        // divider
        g.strokeStyle = "#c9b28a"; g.lineWidth = 1;
        g.beginPath(); g.moveTo(120, 345); g.lineTo(540, 345); g.stroke();
        // body: wrap every paragraph into a flat line list, then draw row by row
        // with a fixed, monotonically increasing y so lines never overlap.
        g.fillStyle = "#33302a"; g.font = "32px 'Kaiti SC','KaiTi','STKaiti',serif";
        ...
        for (var li = 0; li < allLines.length; li++) {
          g.fillText(allLines[li], 330, bodyTop + li * rowH);
        }
        // footer
        g.fillStyle = "#9a8a70"; g.font = "16px sans-serif";
        g.fillText("—— 诗词 · dsh-chinese-poetry ——", 330, 820);
        return cv.toDataURL("image/png");
      } catch (err) { return null; }
    }

    /** Wrap text onto centered canvas lines; returns how many lines were drawn. */
    function wrapCanvasLines(g, text, cx, startY, maxWidth, lineHeight, maxLines) {
      var lines = [];
      var cur = "";
      ...
      for (var j = 0; j < lines.length; j++) g.fillText(lines[j], cx, startY + j * lineHeight);
      return lines.length;
    }
